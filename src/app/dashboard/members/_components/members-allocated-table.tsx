...
export default function AllocatedTable({
  data,
  onRowClick
}: {
  data: StudentBooking[];
  ...
  return (
    <div className="overflow-x-auto">
        ...
        <tbody>
          {data.map((b, idx) => (
            <tr key={b.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => onRowClick(b.id)}>
              ...
              <td className="py-2">{b.allocatedRoomNumber ?? "—"}</td>
      ...
      {data.length === 0 && <div className="p-4 text-center text-gray-500">No members with allocated rooms.</div>}
    </div>
  );
}
